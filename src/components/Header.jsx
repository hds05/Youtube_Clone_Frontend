...
// import ProfileMenu component
import ProfileMenu from "./ProfileMenu";
// import search context
import { useSearch } from "../../context/SearchContext";

function Header({ isOpen, setIsOpen }) {
  // navigate variable for page navigation
  const navigate = useNavigate();
  // Getting login user and logout functions
  const { user, logout } = useAuth();
  // State for opening/closing profile menu
  const [showMenu, setShowMenu] = useState(false);
  // Getting search setter function from context
      ...
      <div className="flex items-center gap-3">
        {user ? (
          <>
            {/* If user logged in */}
            {/* Create */}
            <div className="hidden md:flex items-center gap-1 px-3 py-1 bg-gray-800 rounded-full hover:bg-gray-700 cursor-pointer">
              <FaPlus size={16} />
              <span>Create</span>
            </div>

            {/* Notifications */}
            <div className="p-2 hidden md:block hover:bg-gray-800 rounded-full cursor-pointer">
          ...
          // If not logged In
          <>
            {/* Show static vertical three dots */}
            <BsThreeDotsVertical size={22} className="cursor-pointer" />
            {/* Sign in button. onClick navigate on login page */}
            <Link
              to={"/login"}
