...
function ChannelPage() {
    ...
    useEffect(() => {
        axios.get("http://localhost:3000/channel/mychannel", {
            headers: {
    ...
    if (!channel) {
        return (
            <div className="text-white flex flex-col items-center justify-center h-screen">
                ...
                <Link to={'/createChannel'} className="bg-blue-600 px-4 py-2 rounded-full cursor-pointer">
                    Create Channel
                </Link>
            </div>
        )
    ...
    return (
        <div className="text-white min-h-screen p-4">
            ...
            <div>
                <button className='bg-red-600 text-white fixed bottom-2 right-2 md:bottom-10 md:right-10 p-6 rounded-full cursor-pointer'><MdAdd /></button>
            </div>
        </div>
    )
}
